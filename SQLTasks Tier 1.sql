...
SELECT firstname, surname FROM `Members` LIMIT 10;
...
the member name. */
SELECT DISTINCT CONCAT(Members.firstname," ", Members.surname) AS Member_Name, Facilities.name FROM Bookings 
...
SELECT CONCAT(m.firstname, " ", m.surname) as name, m.recommendedby, r.surname AS recommneder
FROM Members AS m
...
SELECT f.facid AS facility_number, f.name AS facility_name, count(b.memid) AS member_booking
